...
import com.example.imagis.db.ChannelEntity
import com.example.imagis.data.ChannelStore
import com.example.imagis.data.PlaylistSource
...
object SyncEngine {
    ...
    suspend fun syncPlaylists(context: Context, forceSync: Boolean = false): Boolean = withContext(Dispatchers.IO) {
        ...
        // ADAPTATION: Using "iMagisPrefs" to match what SettingsFragment writes to.
        val sharedPref = context.getSharedPreferences("iMagisPrefs", Context.MODE_PRIVATE)
        ...
        for (playlist in allPlaylists) {
            try {
                ...
                if (connection.responseCode == HttpURLConnection.HTTP_OK) {
                    val m3uContent = connection.inputStream.bufferedReader().use { it.readText() }
