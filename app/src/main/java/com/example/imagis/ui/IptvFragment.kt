...
import com.example.imagis.data.ChannelStore
import com.example.imagis.data.IptvChannel
import com.example.imagis.utils.M3uParser
...
class IptvFragment : VerticalGridSupportFragment() {
    ...
    private fun setupFragment() {
        ...
        presenterSelector.addClassPresenter(com.example.imagis.data.PlaylistSource::class.java, PlaylistPresenter())
        ...
        onItemViewClickedListener = OnItemViewClickedListener { _, item, _, _ ->
            if (item is com.example.imagis.data.PlaylistSource) {
                // Open Category
    ...
    private fun loadChannels(urlStr: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    if (channels.isEmpty()) {
                        Toast.makeText(requireContext(), "No channels found in this playlist.", Toast.LENGTH_LONG).show()
                    } else {
    ...
    private inner class PlaylistPresenter : Presenter() {
        ...
        override fun onBindViewHolder(viewHolder: ViewHolder, item: Any) {
            ...
            val playlist = item as com.example.imagis.data.PlaylistSource
