...
class MainFragment : BrowseSupportFragment() {
    ...
    private fun loadTmdbContent(adapter: ArrayObjectAdapter) {
         GlobalScope.launch(Dispatchers.IO) {
             val api = com.epicgera.vtrae.api.TmdbApiClient.service
             val key = com.epicgera.vtrae.api.TmdbApiClient.API_KEY
             val presenter = com.epicgera.vtrae.ui.TmdbCardPresenter()

             // Helper to add a row safely
             suspend fun addRow(headerTitle: String, headerId: Long, fetcher: suspend () -> com.epicgera.vtrae.api.TmdbResponse) {
                 try {
    ...
    // private val allVodItems = java.util.Collections.synchronizedList(mutableListOf<com.epicgera.vtrae.data.VodContent>())
    ...
    private fun loadTMDBRows(adapter: ArrayObjectAdapter) {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                // 1. Trending
                val trending = com.epicgera.vtrae.api.TmdbApiClient.service.getTrending(com.epicgera.vtrae.api.TmdbApiClient.API_KEY)
                addTmdbRow(adapter, "🔥 Trending Now", trending.results, 100)

                // 2. Popular Movies
                val movies = com.epicgera.vtrae.api.TmdbApiClient.service.getPopularMovies(com.epicgera.vtrae.api.TmdbApiClient.API_KEY)
                addTmdbRow(adapter, "🍿 Popular Movies", movies.results, 101)

                // 3. Top TV Series
                val series = com.epicgera.vtrae.api.TmdbApiClient.service.getPopularSeries(com.epicgera.vtrae.api.TmdbApiClient.API_KEY)
    ...
    private suspend fun addTmdbRow(
        ...
        title: String, 
        movies: List<com.epicgera.vtrae.api.Movie>, 
        headerId: Long
    ) {
    ...
    private fun loadVodContent(adapter: ArrayObjectAdapter) {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                com.epicgera.vtrae.data.ChannelStore.globalVodList.clear()
                val localAllItems = mutableListOf<com.epicgera.vtrae.data.VodContent>()
                ...
                val argentinaItems = if (contentArgentina.isNotEmpty()) com.epicgera.vtrae.utils.VodParser.parse(contentArgentina) else emptyList()
                localAllItems.addAll(argentinaItems)
                
                com.epicgera.vtrae.data.ChannelStore.globalVodList.addAll(localAllItems)
        ...
        onItemViewClickedListener = OnItemViewClickedListener { _, item, _, _ ->
            if (item is PlatformMenuOption) {
                ...
                when (item.type) {
                 ...
                 startActivity(intent)
            } else if (item is com.epicgera.vtrae.data.VodContent) {
                // Play VOD
                ...
                startActivity(intent)
            } else if (item is com.epicgera.vtrae.api.Movie) {
            ...
            } else if (item is com.epicgera.vtrae.db.WatchHistoryEntity) {
                ...
                if (!item.videoUrl.isNullOrEmpty()) {
                    ...
                    com.epicgera.vtrae.ui.components.VtrToastManager.showInfo("No saved stream URL for this title.")
                }
            }
        }
    }
    ...
    private inner class VodPresenter : Presenter() {
        ...
        override fun onBindViewHolder(viewHolder: ViewHolder, item: Any) {
            ...
            val vod = item as com.epicgera.vtrae.data.VodContent
    ...
    private inner class FavoriteCardPresenter : Presenter() {
        ...
        override fun onBindViewHolder(viewHolder: ViewHolder, item: Any) {
            val cardView = viewHolder.view as ImageCardView
            val fav = item as com.epicgera.vtrae.db.FavoritesEntity
    ...
    private inner class WatchHistoryCardPresenter : Presenter() {
        ...
        override fun onBindViewHolder(viewHolder: ViewHolder, item: Any) {
            val cardView = viewHolder.view as ImageCardView
            val entry = item as com.epicgera.vtrae.db.WatchHistoryEntity
